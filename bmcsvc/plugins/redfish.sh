...
cmd_logs() {
    ...
    case "${logtype,,}" in
        ...
        info)
            ...
            ;;
        *)
            die "Unsupported IPMI log type: '$logtype' (supported: sel, info)"
            ;;
    esac

    local entries
    entries="$("${ipmi_cmd[@]}" sel elist 2>&1)" \
        || die "Failed to get SEL from node '$node': $entries"

    if [[ "$OUTPUT_FMT" == "json" ]]; then
        #
        # ipmitool does not provide native JSON output.
        # Convert SEL text into basic JSON.
        #
        printf '%s\n' "$entries" |
        awk -F ' \\| ' '
        BEGIN {
        ...
        echo -e "${BOLD}── IPMI SEL: ${node} ─────────────────────────────────────────${RESET}"
        printf "%s\n" "$entries"
    fi
}
# ---------------------------------------------------------------------------
...
cmd_firmware1() {
    ...
    update)
       ...
       #      --data-binary "@${fw_file}" | jq .
         echo "Y" | Yafuflash -nw -ip $BMC_HOST -u $BMC_USER -p $BMC_PASS $fw_file -preserve-config
        ;;
    *)
...
cmd_bmcfw() {
    ...
    status)
        if [[ "$OUTPUT_FMT" == "json" ]]; then
            ...
            #echo -e "${BOLD}── BIOS Firmware Inventory — $name ──────────────────────────${RESET}"
            printf "%-20s %-30s %-20s %-10s\n" "Id" "Name" "Version" "Updateable"
            printf '%0.s─' {1..82}; echo
            rf_get "/UpdateService/FirmwareInventory/BMC" | jq -r '[
                    (.Id//"?"),
                    (.Name//"?"),
                    (.Version//"?"),
                    ((.Updateable//false)|tostring)
...
cmd_biosfw() {
    ...
    status)
        if [[ "$OUTPUT_FMT" == "json" ]]; then
            ...
            printf "%-20s %-30s %-20s %-10s\n" "Id" "Name" "Version" "Updateable"
            printf '%0.s─' {1..82}; echo
            rf_get "/UpdateService/FirmwareInventory/BIOS" | jq --arg BiosVer "$biosver" -r '[
                    (.Id//"?"),
                    (.Name//"?"),
                    (.Version//$BiosVer),
                    ((.Updateable//false)|tostring)
    ...
    update)
       ...
       #      -H "Content-Type: application/octet-stream" \
       #      --data-binary "@${fw_file}" | jq .
         echo "Y" | Yafuflash -nw -ip $BMC_HOST -u $BMC_USER -p $BMC_PASS -d 2 $fw_file -preserve-config
        ;;
    *)
        die "Unknown firmware action: $action  (list | upload <file>)"
        ;;
...
${BOLD}Subcommands:${RESET}
  ...
  ${CYAN}bmcfw${RESET}   <node> <status|upload <file>>
                [--json | --table | --filter <jq-expr>]
               List firmware inventory or upload new firmware.

  ${CYAN}biosfw${RESET}   <node> <status|upload <file>>
                [--json | --table | --filter <jq-expr>]
               List firmware inventory or upload new firmware.
